...
After processing, the resulting \term{profile} data files should be annotated and saved in the database alongside the original raw data files. It is important that the \term{raw} data files remain traceable and that the parameters used in processing are saved so that the process can be repeated and confirmed (see \refer{fig:target_trace}).
...
As new discoveries are made in the area, new standards for the order of the base pairs in a \term{DNA} string are set. This results in a new \term{Genome Release} for a specific species. These are obtained as a set of files which specify the order used in generating \term{profile} data. The researchers need a way to convert \term{profile} data files between different genome releases (see \refer{fig:target_genome}). It is not uncommon for errors in a new release to be discovered after publication. It is therefore also important to store files generated using older genome releases for some time after a new release is published.
