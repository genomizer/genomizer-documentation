...
\begin{figure}[ht]
\addThreeImages{ios_login.png}{ios_loginFailed.png}{ios_changeServer}
\caption{The login screen.}
\label{fig:ios_login}
\end{figure}
\FloatBarrier
After logging in, the user is presented with a search view as seen in the leftmost view in \refer{fig:ios_search}. The bottom menu bar is used to navigate between the Search-, Selected files-, Processes- and More-view in accordance with Apple's current GUI standards for iOS 7. In the current state, the More-menu only contains a logout button which is used to log out. The Selected files-menu contains a list of files selected by the user, sorted by file type.
\begin{figure}[ht]
\addThreeImages{ios_searchFirst.png}{ios_searchSecond.png}{ios_advancedSearch.png}
...
\begin{figure}[ht]
\addThreeImages{ios_searchResultsFirst.png}{ios_selectAnnotations.png}{ios_searchResultsSecond.png}
...
\begin{figure}[htb]
\addTwoImages{ios_filesFirst.png}{ios_filesFileInfo.png}
\caption{The files screen.}
\label{fig:ios_files1}
\end{figure}
\FloatBarrier
The user can also move files to the selected files view. This is done by simply turning the switch to the right of the filename as seen in the left view in \refer{fig:ios_files3} and then pressing the ’Add to Selected files’-button. The user will then be presented with a popup window as shown in the rightmost view in \refer{fig:ios_files3}.

\begin{figure}[htb]
\addTwoImages{ios_filesSecond.png}{ios_addedToSelectedFiles.png}
\caption{The files screen.}
\label{fig:ios_files3}
\end{figure}
\FloatBarrier
If the user presses the ‘Selected Files’-button in the menu, the Selected Files screen is presented, containing all files the user has added to selected files, as seen in the leftmost view in \refer{fig:ios_selectedFiles}. If the user wishes to see more information about a file, it is possible to simply click the blue information sign close to the filename. This shows file information in a similar way as in the files view seen in \refer{fig:ios_files1}. In the selected files screen the user can select files and then press the trashcan icon in the top rightmost corner to delete the currently selected files, as seen in the two rightmost views in \refer{fig:ios_selectedFiles}. The user can also select a task to perform on the currently selected files by pressing the ‘Select task to perform’-button. The user will then be presented with the Select task screen as seen in the leftmost view in \refer{fig:ios_convert}.

\begin{figure}[htb]
\addThreeImages{ios_selectedFilesFirst.png}{ios_selectedFilesSecond}{ios_filesRemoved}
...
\begin{figure}[htb]
\addThreeImages{ios_selectTask.png}{ios_convert.png}{ios_convertSent}
...
If the ‘More’-button in the menu is clicked the More screen is shown as in \refer{fig:ios_more}.  

\begin{figure}[htb]
\addScaledImage{0.17}{ios_more.png}
