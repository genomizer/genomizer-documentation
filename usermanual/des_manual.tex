...
\end{figure}
The application is built with tabs, as illustrated in \refer{fig:des_tabs-view}. Each tab contains separate features of the application. There are six tabs: Search, Upload, Process, Workspace and Administration. One tab that is designed and planned, but not implemented is Analyze.
\begin{figure}[htb]
	\addImage{tabs.png}
...
\subsection{Search}
The first tab that meets the user after logging in is the Search tab, illustrated in \refer{fig:des_search-query}. The Search tab uses the same syntax as the “Pubmed Advanced Search Builder”\cite{des_pubmed_query}. It has a text field where you can type in the search query yourself, or you can use the query builder below it.

A query can be built by choosing an annotation type, eg. \term{Species} and a value to search for, eg. \term{Human}. Additional search parameters can be added using the plus button and require one of the logical expressions \term{AND, OR} or \term{NOT}. This specifies how hits on the parameter are treated.

The example in \refer{fig:des_search-query} would return all experiments on human DNA that are also annotated as female. If the \term{AND} was replaced by \term{OR} it would return all experiments on human DNA together with all experiments annotated as female. Similarly, if the \term{AND} was replaced by \term{NOT} it would return all experiments on human DNA that are not annotated as female.
\begin{figure}[htb]
...
\subsection{Upload}
Uploading files to the database is done through the \term{Upload} tab.
...
When the user presses the new experiment button they are presented with the annotations they can use to provide additional information about the experiment. Annotations in bold text are \term{forced} and need to be provided.

If the name of an existing experiment is entered users can easily upload more files to that experiment by clicking the \term{Search for existing experimnet} button. The user is presented with the buttons \term{Select files} and \term{Upload selected files}. Files can be selected using the file browser, shown in \refer{fig:des_upload}, or by dragging and dropping files from a separate window. Selected files are uploaded by clicking the \term{Upload selected files} button. The upload tab is illustrated in \refer{fig:des_upload-view}.
